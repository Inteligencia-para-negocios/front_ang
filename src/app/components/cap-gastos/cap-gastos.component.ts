...
export class CapGastosComponent implements OnInit {
  ...
  captureForm = new FormGroup({
    ...
    partida: new FormControl({ value: '', disabled: true }, [
      Validators.required,
    ]),
    proveedor: new FormControl({ value: '', disabled: true }),
    efectivoSol: new FormControl({ value: '', disabled: true }, [
      Validators.required,
    ]),
    area: new FormControl({ value: '', disabled: true }),
    empresaSol: new FormControl({ value: '', disabled: true }, [
      Validators.required,
    ]),
    financiamiento: new FormControl({ value: '', disabled: false }, [
      Validators.required,
    ]),
    tipoDeGasto: new FormControl({ value: '', disabled: true }),
    justificacion: new FormControl({ value: '', disabled: true }, [
      Validators.required,
    ]),
    efectivoComprobado: new FormControl(
      localStorage.getItem('idUser') || localStorage.getItem('idUser')
    ),
  ...
  cargarDatos(comprobacion: any) {
    ...
    if (comprobacion.recurrente) recurrente = 'RECURRENTE';
    else recurrente = 'NO RECURRENTE';
  ...
  capturaGasto() {
    ...
    formData.append(
      'monto',
      this.captureForm.value.efectivoComprobado?.toString() || ''
    );
    formData.append(
      'idEmpresa',
      this.captureForm.value.financiamiento?.toString() || ''
    );
