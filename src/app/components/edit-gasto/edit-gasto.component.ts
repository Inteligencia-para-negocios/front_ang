  ...
  getGasto() {
    ...
    this.captureForm = new FormGroup({
      sucursal: new FormControl(
        { value: gasto[0]['Sucursal'], disabled: true },
        Validators.required
      ),
      provedor: new FormControl(
        { value: gasto[0]['Provedor'], disabled: true },
        Validators.required
      ),
      cheque: new FormControl(
        { value: gasto[0]['Chequera'], disabled: true },
        Validators.required
      ),
      efectivoLib: new FormControl(
        { value: gasto[0]['Liberado'], disabled: true },
        Validators.required
      ),
      area: new FormControl(
        { value: gasto[0]['Area'], disabled: true },
        Validators.required
      ),
      efeCo: new FormControl('', [
        Validators.required,
        Validators.pattern('[0-9]*'),
      ]),
      concepto: new FormControl(
        { value: gasto[0]['Concepto'], disabled: true },
        Validators.required
      ),
      responsable: new FormControl(
        { value: gasto[0]['Responsable'], disabled: true },
        Validators.required
      ),
      justificacion: new FormControl(
        { value: gasto[0]['Justificacion'], disabled: true },
        Validators.required
      ),
      revolvente: new FormControl({
        value: gasto[0]['idRevol'],
        disabled: true,
      }),
    });
  }
