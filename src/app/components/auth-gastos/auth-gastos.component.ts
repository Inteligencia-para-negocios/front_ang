...
import { GastoRecurrenteService } from '../../services/gasto-recurrente.service';
...
export class AuthGastosComponent implements OnInit {
  ...
  onChangeStatus(event: Event, solicitud: any): void {
    ...
    const select = this.status.find(
      (st) => st.idCatalogo === selectElement.value
    );
    const obj = {
      idSolicitud: solicitud.idSolicitud,
      estatus: selectElement.value,
    };
    if (select.nombre === 'AUTORIZADO') {
      this.solicitud.authSolicitud(obj).subscribe({
        next: (data: any) => {
          ...
          this.gasto
            .createGasto({ solicitud: solicitud.idSolicitud })
            .subscribe({
              next: (data: any) => {
