...
export class GastCompComponent implements OnInit {
  ...
  constructor(private CHEQUE: CapChequeService) {}
  ...
  ngOnInit() {
    ...
    if (
      localStorage.getItem('rol') === 'contabilidad' ||
      localStorage.getItem('rol') == 'contabilidad' ||
      localStorage.getItem('rol') === 'visualizador' ||
      localStorage.getItem('rol') == 'visualizador'
    ) {
  ...
  entregaContabilidad() {
    ...
    this.CHEQUE.entregaContabilidad(cheque).subscribe((response) => {
      Swal.fire({
        position: 'center',
        icon: 'warning',
